...
import 'package:futbol_analiz_app/match_detail_screen.dart';
...
class CompactMatchCard extends StatelessWidget {
  ...
  const CompactMatchCard({super.key, required this.matchData});
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: InkWell(
        onTap: () {
          Navigator.push(context, MaterialPageRoute(builder: (context) => MatchDetailScreen(matchData: matchData)));
        },
...
class _MatchesView extends ConsumerWidget {
      ...
      child: Stack(
        ...
        children: [
          ...
          Padding(
            ...
            child: Text(
              "Seçilen liglerde şu anda canlı karşılaşma bulunmuyor.",
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: state.allMatches.when(
            data: (allMatches) {
              final leaguesToShow = state.activeLeagueIds.isEmpty 
                  ? state.selectedCompetitionIds 
                  : state.activeLeagueIds;
              final filteredList = _filterMatches(allMatches, leaguesToShow, ref);
              
              final live = filteredList.where((m) => ['1H', 'HT', '2H', 'ET', 'P', 'LIVE'].contains(m['fixture']['status']['short'])).toList();
              ...
              if (filteredList.isEmpty && !state.allMatches.isLoading) {
                 final dateString = DateFormat('dd MMMM yyyy', 'tr_TR').format(state.selectedDate);
                 return Center(child: Padding(
                   padding: const EdgeInsets.all(24.0),
                   child: Text("$dateString tarihi için seçili kriterlere uygun maç bulunamadı.", textAlign: TextAlign.center),
                 ));
              }
              ...
              return RefreshIndicator(
                ...
                child: CustomScrollView(
                  slivers: [
                    ...
                    live.isEmpty
                        ? SliverToBoxAdapter(child: _buildEmptyLiveCard(context))
                        : _buildMatchSliverList(live, ref, isFeatured: true),
                    _buildSectionHeader(context, "Yaklaşan Maçlar"),
                    upcoming.isEmpty
                        ? const SliverToBoxAdapter(child: _EmptyStateMessage(message: "Seçili ligler için bugün başka maç bulunmuyor."))
                        : _buildMatchSliverList(upcoming, ref),
                    _buildSectionHeader(context, "Biten Maçlar"),
                    finished.isEmpty
                        ? const SliverToBoxAdapter(child: _EmptyStateMessage(message: "Seçili ligler için bugün biten maç bulunmuyor."))
                        : _buildMatchSliverList(finished, ref),
                  ],
                ),
              );
            },
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (error, stack) => Center(child: Text("Maçlar yüklenemedi: $error")),
          ),
        ),
      ],
    );
  ...
  Widget _buildMatchSliverList(List<dynamic> matches, WidgetRef ref, {bool isFeatured = false}) {
    final controller = ref.read(matchesControllerProvider.notifier);
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate(
          (context, index) {
            ...
            return CompactMatchCard(matchData: match);
          },
  ...
  List<dynamic> _filterMatches(List<dynamic> allMatches, Set<int> activeLeagueIds, WidgetRef ref) {
    if (activeLeagueIds.isEmpty) {
      final favoriteIds = ref.read(matchesControllerProvider).selectedCompetitionIds;
      return allMatches.where((match) => favoriteIds.contains(match['league']['id'])).toList();
    }
